...
def Deviceall(request):
    ...
    if request.method == 'GET':
        alldevice = DeviceAll.objects.all() 
        serializer = DeviceSerializer(alldevice,many=True)
        return Response(serializer.data,status=status.HTTP_200_OK)
...
def register_app(request):
    if request.method == 'POST':
        ...
        if username != '' and password != ''and firstname != '':
            ...
            if userclass == 'User':
                try:
                    user = User.objects.create_superuser(username = username, password = password,firstname = firstname)
                    user.save()
...
@api_view(['GET', 'POST'])
def get_somedata(request, parameter):
    queryset = DeviceAll.objects.values('spo2', 'create','hr','temp').order_by('-id')[:1]
    if parameter == 'parameterall':
...
# @api_view(['GET', 'POST'])
# def get_spo2(request, graphspo2):
#     queryset = DeviceAll.objects.values('spo2').order_by('-id')[:20]
#     if graphspo2 == 'spo2graph':
#         serializer = Spo2Serializer(queryset, many = True)
